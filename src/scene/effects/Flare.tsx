...
const SMOKE_VERT = /* glsl */ `
  ...
  void main(){
    ...
    p.y = rise;
    // 上升中受风偏移并发散
    p.x += uWind.x * rise * (0.8 + aSeed) + sin(mod(aSeed * 30.0 + uTime, 6.283185307179586)) * life * 4.0;
    p.z += uWind.y * rise * (0.8 + aSeed) + cos(mod(aSeed * 22.0 + uTime, 6.283185307179586)) * life * 4.0;
...
const EMBER_VERT = /* glsl */ `
  ...
  void main(){
    ...
    p.y = rise;
    p.x += uWind.x * rise * 0.5 + sin(mod(aSeed * 40.0 + uTime * 6.0, 6.283185307179586)) * life * 2.0;
    p.z += uWind.y * rise * 0.5 + cos(mod(aSeed * 33.0 + uTime * 6.0, 6.283185307179586)) * life * 2.0;
    vec4 mv = modelViewMatrix * vec4(p, 1.0);
